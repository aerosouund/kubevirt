...
func (c *controlledDevice) Start() {
	...
	logger.Infof("ammar: Starting a device plugin for device: %s", deviceName)
	retries := 0
	...
	go func() {
		for {
			...
			if err != nil {
				...
				log.DefaultLogger().Info("ammar: device started successfully")
				retries = 0
			}
...
func (c *DeviceController) startDevice(resourceName string, dev Device) {
	log.DefaultLogger().Info("ammar: device plugin starting from the unexported method")
	c.stopDevice(resourceName)
